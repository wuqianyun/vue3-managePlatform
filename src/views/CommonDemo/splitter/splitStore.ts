import type splitPane from "./splitPane";
...
export type splitPane = typeof splitPane & Pane;
export interface splitState {
  panes: Array<splitPane>; // 所有 pane 对象的一些关键信息
  paneCount: number;
...
export class splitStore {
  ...
  setPanes({ panes }: { panes: splitPane[] }): void {
    this.state.panes = panes.map((pane: splitPane, index: number) => {
      if (pane.component) {
  ...
  getPane(index: number): splitPane {
